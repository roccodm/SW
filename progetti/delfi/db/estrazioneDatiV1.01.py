...
connection = psycopg2.connect(user = "rocco",password = "XXXXXXXXXXXX",host = "localhost",port = "5432",database = "delfi")
        ...
        if data["OperationalData"][section].get("LATITUDE (gg.ddddd)")!="":
            ...
            if not errorIssue:
                ...
                for key in r["species"]:
                    ...
                    val2=r["species"][key].get("NON-MARKETABLE_INDIVIDUALS_WEIGHT_g")
                    if val2==None: 
                        tc_nm_kg=0
                    else: 
                        tc_nm_kg+=float(val2)
                record["TC"]=tc
                record["TC_NM_KG"]=tc_nm_kg
